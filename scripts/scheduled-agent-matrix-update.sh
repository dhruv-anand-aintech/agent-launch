...
Rules:
- Address only the matrix dataset/site artifacts. Do not edit root `README.md`, `bin/agent-launch`, unrelated scripts, or unrelated project files.
...
- Create a GitHub PR with `gh pr create --base main --head <current-branch> --title "Scheduled coding agent matrix update"` and a body that includes summary, changed agents/attributes, blocked sources, and the three validation commands.
